...
from logic.logic_services.parameter_logic import insert_update_param_by_name, get_all_params
...
def set_user_info():
    ...
    gpalgooutput = GeneralParam('AlgorithmOutput', "AlgoOutput")  # TODO get from user
    gpalgoversion = GeneralParam('AlgorithmVersion', '1.4')  # TODO get from user
    algorunoptimization = GeneralParam('RunOptimization', '1')  # TODO get from user
    videofolder = GeneralParam('VideoFolder', "C:/testhomage/")  # TODO get from user
    ...
    # set params
    x = Parameter('x', 1, 4, 1, 2)  # TODO get from user
    y = Parameter('y', 6, 9, 1.5, 6)  # TODO get from user
    z = Parameter('z', 100, 104, 2, 100)  # TODO get from user
    insert_update_param_by_name('x', x)
    insert_update_param_by_name('y', y)
    insert_update_param_by_name('z', z)
    # set videos from path
...
def save_avgscore_to_file(algooutput, algoversion, avgscore, cycleid, videospath):
...
def run_cycle():
    ...
    for params in permutationlist:
        ...
        save_avgscore_to_file(algooutput, algoversion, avgscore, cycleid, videospath)
